...
const form =  useForm({id:props.product.id,name:props.product.name,name_en:props.product.name_en,
    group:null,type:null,category:null,text:props.product.text,status:0,
    sings:props.product.sings,design_type:props.product.design_type,prerequisites:[],
    prerequisite_version:[],additional_facilities:[],
...
const submitTime = ()=>{
    Inertia.visit(route('product.edit',form.id),{ only: [errors.value,hasErrors.value,props.alert] })
}
const submit = () => {
                                ...
                                <div class="card-body">
                                    <div class="row gx-2">
                                        ...
                                        <div class="col-lg-6" v-if="form.prerequisites" v-for="prerequisite,index in form.prerequisites" :key="index" >
                                            <label class="form-label">نسخه پیش نیازها محصول ({{prerequisite.name}})</label>
                                            <select v-model.lazy="form.prerequisite_version[prerequisite.id]" class="form-select">
                                                <template v-for="child,index in prerequisite_version" :key="index">
                                                    <option v-if="prerequisite.id == child.parent_id"  :value="child[child.id]" >{{ child.name }}</option>
                                                </template>
                                            </select>
                                        </div>
                                    </div>
                                        ...
                                        <div class="col-lg-6" v-if="form.test" v-for="tes,index in form.test" :key="index" >
                                            ...
                                            <select v-model.lazy="form.sub_test[tes.id]" class="form-select">
                                                <template  v-for="child,index in test_version" :key="index">
                                                    <option v-if="tes.id == child.parent_id" :value="child[child.id]" >{{ child.name }}</option>
                                                </template>
                                            </select>
                                        </div>
                                    </div>
