...
const submitTime = ()=>{
    Inertia.visit(route('productAdmin.create'),{ only: [errors.value,hasErrors.value,props.alert] })
}
    ...
    <div class="col-sm-12 mt-3" v-for="(ticket,index) in props.replies" :key="index" >
        ...
        <div class="mb-4 me-5">
            <div class="d-flex">
                ...
                <a v-if="ticket.user.id !== props.users.id && ticket.status !== 3" :href="'#CollapseExample'+ticket.id" class="reply me-1" data-bs-toggle="collapse" aria-expanded="false" :aria-controls="'CollapseExample'+ticket.id">پاسخ</a>
            </div>
        </div>
