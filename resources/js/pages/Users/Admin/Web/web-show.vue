...
const props = defineProps({
    ...
    notifications:Object,companies:Object,descriptions:Object,path:String,alert:Object, menus: Object,webDesign:Object,
    cart:Object
});

const form =  useForm({
    name:props.webDesign.name,name_en:props.webDesign.name_en,group:null,type:null,
    category:null,id:props.webDesign.id,status:props.webDesign.status,price:props.webDesign.price,
    text:props.webDesign.text,image:props.webDesign.image,
});
...
const submitTime = ()=>{
    Inertia.visit(route('web_design.show',[form.id]),{ only: [errors.value,hasErrors.value,props.alert] })
}
const submit = () =>{
    ...
    {
        form.post(route('web_design.store'),{
            onFinish:() => submitTime()
        })
    }
};
            ...
            element.routes.forEach(route => {
                if(route.name == props.path)
                {
                    element.sections.forEach(section => {
                        if(section.name == 'web_designs' && props.webDesign.group.id == element.id)
                        {
                ...
                child.routes.forEach(route => {
                    if (route.name == props.path)
                    {
                        if (child.sections.length > 0)
                        {
                            ...
                            {
                                if(section.name == 'web_designs' && props.webDesign.type.id == child.id)
                                {
                ...
                if(child.routes.length > 0)
                {
                    child.routes.forEach( route => {
                        if (route.name == props.path)
                        {
                            ...
                            {
                                child.sections.forEach(section => {
                                    if(section.name == 'web_designs' && props.webDesign.category.id == child.id )
                                    {
                                    ...
                                    <div class="col-lg-6">
                                        <div class="mt-4">
                                            ...
                                            <div class="input-upload">
                                                <img  v-if="form.image && form.image.url" class="img mb-3" :src="$page.props.ziggy.url+'/storage/'+ form.image.url" :alt="form.name" />
