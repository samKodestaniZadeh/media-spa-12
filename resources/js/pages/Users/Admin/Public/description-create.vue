...
const form =  useForm({subject:null,text:null});
...
const submitTime = ()=>{
    Inertia.visit(route('description.create'),{ only: [errors.value,hasErrors.value,props.alert] })
}
...
const submit = () =>{
    ...
    {
        form.post(route('description.store'),{
            onFinish:() => submitTime()
        })
    }
};
