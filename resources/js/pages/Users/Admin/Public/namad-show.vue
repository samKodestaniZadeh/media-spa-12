...
const submitTime = ()=>{
    Inertia.visit(route('namadAdmin.show',form.id),{ only: [errors.value,hasErrors.value,props.alert] })
}
const submit = () =>{

    if( form.tag == null && form.title == null)
    {
        let text
    ...
    {
        form.post(route('namadAdmin.store'),{
            onFinish:() => submitTime()
        })
    }
};
