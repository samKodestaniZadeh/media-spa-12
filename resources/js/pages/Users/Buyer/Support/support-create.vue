...
const props = defineProps({
    menus:Object,orders:Object,users:Object,notifications:Object,wallet:Number,
    companies:Object,descriptions:Object,tarahis:Object,path:String,alert: Object,
    cart:Object
});
...
const submitTime = ()=>{
    Inertia.visit(route('support.create'),{ only: [errors.value,hasErrors.value,props.alert] })
}
...
const next = () => {
    ...
    else if (step.value == 'step2')
    {
        ...
        {
            if (form.recepiant && form.subject && form.product) {
                step.value = 'step3'
                subs.value = null
            }
            else
...
if (props.orders) {
    props.orders.forEach(order => {
        ...
        order.products.forEach(product => {
                ...
                order.sub_order.forEach(element => {

                if (element.link && element.link.link) {
        ...
        <main class="main-wrap rtl">
            <section class="content-main">
                ...
                <div class="card mb-4">
                            ...
                            <div class=" mb-4">
                                <div class="d-flex" >
                                    ...
                                    <div class="col-sm-4 me-1"  id="product" v-if="subs == true">
                                        <label  class="form-label">محصول / خدمات<span class="text-danger me-1">*</span></label>
                                        <select v-model.lazy="form.product" name="product" class="form-select"  aria-label="select example">
                                                <option v-for="(order,index) in orders" :key="index" :value="order">
                                                    <template v-if="order.title">{{ order.title }}</template>
                                                    <template v-if="order.name">{{ order.name }}</template>
                                                </option>
                    ...
                    <div class="card-body" v-if="step == 'step' || subs == true && form.product || subs == false">
