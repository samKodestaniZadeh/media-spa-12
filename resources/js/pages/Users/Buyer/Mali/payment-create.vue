...
const props = defineProps({
    auth:Object,cart:Object,banks:Object,users:Object,menus:Object,notifications:Object,now:String,
    companies:Object,descriptions:Object,path:String,alert:Object,wallet:Number,cart:Object
});
...
const submitTime = ()=>{
    Inertia.visit(route('payment.create'),{ only: [errors.value,hasErrors.value,props.alert] })
}
...
const submit = ()=>{
    ...
    else if(form.transaction == 'برداشت' && form.price > props.users.profile.wallet){
            let text
                                        ...
                                        <div class="row">
                                            <div class="col-lg-12">
                                                <div class="row gx-3">
                                                    ...
                                                    <div class="col-3 mb-3" >
                                                        <label class="form-label">حساب بانک<span class="text-danger">*</span></label>
                                                        <select v-model="form.bank" class="form-select" name="bank" id="" autocomplete="bank">
                                                            <option v-if="banks.length > 0"  v-for="(bank,index) in banks" :key="index" :value="bank">{{bank.menu.name}}-{{bank.shaba_number}}</option>
                                                            <option  v-else disabled> گزینه ای یافت نشد.</option>
                                                        </select>
                                                    </div>
                                                    <div class="col-lg-3 mb-3" >
                                                        <label class="form-label">تاریخ<span class="text-danger">*</span></label>
                                                        <date-picker v-model="form.date" format="YYYY-MM-DD HH:mm:ss" display-format="dddd jDD jMMMM jYYYY"  disable="Friday" color="#1ABC9C" :min="props.now" type="date"></date-picker>
                                                    </div>
                                                    <div class="col-lg-3 mb-3" >
